...
```{r}
survey.clean <- survey
for(row in 1:dim(survey)[1]) {
  for(col in 2:dim(survey)[2]) {
    if(survey[row,col]< 1 | survey[row,col]>5) {
      survey.clean[row,col]<- NA
    }
  }
}
survey.clean
...
```{r}
for(p in 1:dim(survey)[2]){
  survey.clean$invalid.answers[p] <- sum(is.na(survey.clean[p,]))
}
survey.clean
```